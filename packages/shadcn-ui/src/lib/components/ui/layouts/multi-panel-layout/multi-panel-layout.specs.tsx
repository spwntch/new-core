...
export const playFlip = async ({
  ...
  const subBlockContainer =
    mainBlock.parentElement?.previousElementSibling ||
    mainBlock.parentElement?.nextElementSibling;
  expect(subBlockContainer).toBeInTheDocument();
  expect(subBlockContainer?.firstChild).toHaveTextContent(
    'Sub Block 1 Content'
  );
  expect(subBlockContainer?.childNodes[1]).toHaveTextContent(
    'Sub Block 2 Content'
  );
  if (subBlock3) {
    expect(subBlockContainer?.lastChild).toHaveTextContent(
      'Sub Block 3 Content'
    );
  }
...
export const playTwoSubBlocks = async ({
  canvasElement,
}: {
  canvasElement: HTMLElement;
}) => {
...
export const playOneSubBlock = async ({
  canvasElement,
}: {
  canvasElement: HTMLElement;
}) => {
