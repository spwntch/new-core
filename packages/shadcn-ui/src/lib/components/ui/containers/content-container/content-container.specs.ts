...
export const playBasic = async ({
  ...
  const announcement = await canvas.findByText(
    'Spawntech is transforming software development'
  );
  ...
  const paragraph1 = await canvas.findByText(
  ...
  const paragraph2 = await canvas.findByText(
    'Innovation at unparalleled speed.'
  );
  const paragraph3 = await canvas.findByText(
