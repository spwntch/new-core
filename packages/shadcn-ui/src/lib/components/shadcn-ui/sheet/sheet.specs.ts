...
const openSheetAndCheckContent = async (
...
export const playBasic = async ({
  ...
  await openSheetAndCheckContent(
    canvasElement,
    'Sheet Title',
...
export const playWithHelpContent = async ({
  canvasElement,
}: {
  canvasElement: HTMLElement;
}) => {
  ...
  const sheetDescription = await canvas.findByText(
