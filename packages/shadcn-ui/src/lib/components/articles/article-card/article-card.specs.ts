...
export const playBasic = async ({
  ...
  const subtitle = await canvas.findByText(
    "You've heard the term, but here's the real reason that not having one is holding you back."
  );
...
export const playNoTags = async ({
  ...
  // Check if the subtitle is present
  const subtitle = await canvas.findByText(
    'How Digital Experience Drives Customer Engagement'
  );
  expect(subtitle).toBeInTheDocument();
